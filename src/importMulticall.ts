...
export async function importMulticall(chainId: number, preset: string): Promise<{ address: string; abi: string[] }> {
  ...
  switch (deployment) {
    ...
    default: {
      throw new Error(`Unsupported deployment ${deployment} for Multicall`);
    }
  }
}
