...
> {
  ...
  switch (deployment) {
    case '1-main': {
      const [{ default: rewardsDistributors }] = await Promise.all([import('@synthetixio/v3-contracts/1-main/rewardsDistributors.json')]);
    ...
    case '10-main': {
      const [{ default: rewardsDistributors }] = await Promise.all([import('@synthetixio/v3-contracts/10-main/rewardsDistributors.json')]);
      return rewardsDistributors;
    }
    case '8453-andromeda': {
      const [{ default: rewardsDistributors }] = await Promise.all([
